...
export function ServicesRail({ ready }: { ready: boolean }) {
  ...
  useGSAP(
    () => {
      ...
      mm.add("(min-width: 768px) and (prefers-reduced-motion: no-preference)", () => {
        const distance = () => Math.max(track.scrollWidth - window.innerWidth + 64, 400);
  ...
  return (
    <section id="services" ref={root} className="paper-section relative overflow-hidden">
      ...
      <div className="h-track mt-12 flex w-max gap-5 px-5 pb-24 md:mt-16 md:gap-7 md:px-8 md:pb-28">
        {site.services.map((service, index) => (
          ...
          >
            ...
            <div>
              ...
              <p className="mt-3 text-sm leading-relaxed text-mist md:text-base">{service.copy}</p>
            </div>
          </article>
        ))}
      </div>
